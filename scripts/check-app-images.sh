...
assert api_config_from_env({"PILOT107_POSTGRES_DSN": "postgresql://postgres@db/pilot107"}).postgres_dsn
assert worker_config_from_env({"PILOT107_WORKER_BACKEND": "demo"}).backend == "demo"
assert worker_config_from_env({"PILOT107_POSTGRES_DSN": "postgresql://postgres@db/pilot107"}).postgres_dsn
