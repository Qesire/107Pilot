...
def main() -> int:
    ...
    worker = RuntimeReconcileWorker(
        ...
        worker_id="smoke-api-run-get-worker",
    )
    run = service.submit(
        RunSubmitRequest(
            owner="alice",
    ...
    thread.start()
    try:
        payload = _get_json(f"http://127.0.0.1:{port}/api/v1/runs/{run.run_id}")
    finally:
        server.shutdown()
        server.server_close()
    ...
    if (
        ...
        or payload["exit_code"] != "0:0"
        or payload["collection_state"] != "succeeded"
        or not payload["job_id"]
    ):
        print(f"unexpected api run result: {payload}", file=sys.stderr)
        return 1

    print(
        f"api run get smoke {payload['run_id']} job={payload['job_id']} "
        f"state={payload['state']} collection={payload['collection_state']}"
    )
    print("url=" + f"http://127.0.0.1:{port}/api/v1/runs/{run.run_id}")
    return 0
