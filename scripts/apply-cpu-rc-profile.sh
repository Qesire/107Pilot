...
for _ in $(seq 1 60); do
  if "${compose[@]}" exec -T slurmctld sacctmgr -n list cluster >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
...
run_sacctmgr() {
  "${compose[@]}" exec -T slurmctld sacctmgr -i "$@" >/dev/null
}
...
run_sacctmgr modify user where user=alice account=competition cluster="$cluster_name" set QOS=qos_cpu_rc DefaultQOS=qos_cpu_rc || true

"${compose[@]}" exec -T slurmctld scontrol reconfigure >/dev/null
profile="$("${compose[@]}" exec -T slurmctld sacctmgr -nP show assoc where user=alice format=User,Account,QOS,DefaultQOS)"
